...
const BookingDetail = () => {
  ...
  const id = slug; // Use slug as the id for API calls
  const navigate = useNavigate();
  
  // DEBUG LOGS
  console.log("=== CUSTOMER BOOKING DETAIL DEBUG START ===");
  console.log("🔍 1. Route slug from useParams:", slug);
  console.log("🔍 2. Using as ID:", id);
  console.log("🔍 3. ID type:", typeof id);
  console.log("🔍 4. ID length:", id?.length);
  console.log("🔍 5. Current URL:", window.location.href);
  console.log("🔍 6. ID is valid MongoDB ObjectId format:", /^[0-9a-fA-F]{24}$/.test(id));
  ...
  const customRefetch = React.useCallback(() => {
    ...
    console.log("🚀 CUSTOMER CUSTOM FETCH - Starting API request for order:", id);
    ...
    .then(response => {
      console.log("🚀 Customer custom fetch response status:", response.status);
      console.log("🚀 Customer custom fetch response ok:", response.ok);
      if (!response.ok) {
    ...
    .then(data => {
      console.log("🚀 Customer custom fetch response data:", data);
      const order = data.order || data;
      console.log("🚀 Customer extracted order:", order);
      setFetchState({
        data: order,
        isLoading: false,
        isFetching: false,
    ...
    .catch(error => {
      console.log("🚀 Customer custom fetch error:", error);
      setFetchState({
        data: null,
        isLoading: false,
        isFetching: false,
  ...
  const worker = order.sellerId || {}; // Changed from workerId to sellerId
  const gig = order.gigId || {}; // Get gig information

  // Debug log successful data extraction
  if (order) {
    console.log("✅ CUSTOMER ORDER LOADED SUCCESSFULLY");
    console.log("✅ Worker (sellerId):", worker);
    console.log("✅ Gig (gigId):", gig);
    console.log("✅ Order status:", order.status);
    console.log("✅ Order date:", order.date);
    console.log("✅ Order timeSlot:", order.timeSlot);
    console.log("✅ Order jobDescription:", order.jobDescription);
  }
  ...
  return (
    <div className="min-h-[80vh] bg-white">
      ...
      <Hero
        ...
        subtitle={gig.title || "Service Booking"} // Changed from order.serviceName to gig.title
        place="Customer"
      />
        ...
        <section className="lg:col-span-2 space-y-5">
          ...
          <div className="rounded-2xl border border-neutral-200 shadow-sm bg-white p-5">
            ...
            {order.jobDescription && (
              <p className="text-sm text-gray-600 mt-2">{order.jobDescription}</p>
            )}
